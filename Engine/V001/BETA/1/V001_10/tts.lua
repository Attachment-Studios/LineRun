...
function tts:say(string)
    ...
    if (not tts.sources[string]) then
        if (not love.filesystem.getInfo("cache/utility/tts")) then
            love.filesystem.createDirectory("cache/utility/tts")
        end

        local file_name = "cache/utility/tts/" .. string:gsub("%s", "_") .. ".mp3"
